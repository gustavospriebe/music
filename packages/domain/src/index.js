...
export const calculatePriceCents = (baseCents, additionsCents = []) => {
...
export const hashToken = (token, pepper) => createHash('sha256').update(`${pepper}:${token}`).digest('hex');
export const createAccessToken = () => randomBytes(32).toString('base64url');
export const verifyToken = (token, expectedHash, pepper) => {
...
const unsafePatterns = [
    ...
    {
        category: 'copyright',
        pattern: /\b(copie|copiar|igualzinha?|igual)\b.{0,80}\b(m[uú]sica|letra|can[cç][aã]o)\b|\b(letra da m[uú]sica|m[uú]sica conhecida|can[cç][aã]o conhecida)\b/i,
    },
    {
...
export const evaluateContent = (input) => {
...
const normalize = (value) => value
    ...
    .replace(/\s+/g, ' ')
    .trim();
const isRepresented = (text, value) => {
    const normalizedValue = normalize(value);
    if (normalizedValue.length < 3)
        return true;
    return text.includes(normalizedValue);
};
export const validateLyrics = (lyrics, story) => {
    const errors = [];
    const fullLyrics = normalize(lyrics.fullLyrics);
    if (!fullLyrics)
        errors.push('A letra não pode estar vazia.');
    ...
    if (lyrics.sections.length < 3)
        errors.push('A letra precisa ter pelo menos três seções.');
...
export const makeMusicPrompt = (lyrics) => [
...
export const retryDelayMs = (attempt, baseDelayMs = 1_000, maxDelayMs = 60_000, random = Math.random) => {
