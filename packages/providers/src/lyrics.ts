...
export const musicalStory = (story: CreativeBrief) => creativeBriefSchema.parse(story);

export const createOpenRouterLyricsProvider = (config: LyricsProviderConfig): LyricsProvider => ({
  generate: async (story, feedback, refinement, signal) => {
    if (!config.apiKey || !config.model)
      throw new AiProviderError(
        'AI_CONFIGURATION_MISSING',
        unknownAiUsage(config.model, Date.now()),
        'failed',
      );
    ...
    let usage = unknownAiUsage(model, startedAt);
    try {
          ...
          messages: [
            {
              ...
              content: [
                ...
                'subjectName é o tema ou inspiração e brief é uma direção criativa: transforme o contexto livremente em versos, sem exigir nomes ou temas literais nem copiar o briefing literalmente ou inventar detalhes pessoais. Somente fatos explicitamente incluídos em facts são trechos obrigatórios; a lista pode estar vazia.',
                ...
                'Regras obrigatórias: 3 a 16 seções incluindo pelo menos um "chorus"; incorpore o tema naturalmente sem exigir transcrição literal do subjectName; TODOS os itens de facts devem aparecer em fullLyrics com as MESMAS PALAVRAS (preserve a ordem das palavras; pode inserir quebras de linha e rimar ao redor, sem parafrasear ou omitir); use o genre, mood e voice informados na história; não imite artistas, não invente fatos, não inclua assuntos proibidos.',
      ...
      if (!response.ok) {
        ...
        throw aiHttpError(response.status, unknownAiUsage(model, startedAt));
      }
      ...
      usage = reportedAiUsage(model, startedAt, body.id, body.usage);
      try {
