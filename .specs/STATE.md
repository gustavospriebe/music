...
- **Execução atual**: [external-activation](features/external-activation/spec.md); [evidências externas](features/external-activation/validation.md). Homologação técnica executada; [PR #2](https://github.com/gustavospriebe/music/pull/2) integrada em `efb0f1789adbebc9beea02e92d538b885fe17544`. Este fechamento documental posterior não altera código executável.
- **Git/CI**: remediação integrada à `main`; WIP preservado. CI do HEAD da PR (runs 34706072351/34706074833) e do merge `efb0f17` (run 34706291941) PASS: 487 Vitest + um operacional, 48 E2E, format/lint/typecheck/build e sensor Docker. O merge tem árvore idêntica ao HEAD testado `7092ebb`. Checkout local sincronizado, sem WIP no encerramento da implementação.
- **Railway comprovado**: web/API/worker SUCCESS no mesmo SHA `efb0f1789adbebc9beea02e92d538b885fe17544`. O trigger aguardou o CI; worker pulado por arquivos inalterados foi promovido explicitamente nesse mesmo SHA. Health da API e do proxy200; comércio fechado e sandbox confirmados. PostgreSQL18.6, journal0000–0016 com 17 hashes iguais ao repo; sem nova migration após essa inspeção. CI obrigatório e pre-deploy único mantidos.
...
- **Encerramento operacional**: acesso anônimo aos três objetos respondeu403; variável web obsoleta removida; sessão admin encerrada e chave SSH temporária revogada/removida. Dumps privados preservados fora do repo.
